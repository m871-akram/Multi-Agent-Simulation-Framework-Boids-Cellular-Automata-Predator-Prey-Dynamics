...
/**
 * Classe abstraite qui représente un système générique de boids.
 * Elle factorise tous les attributs et méthodes communes à tous les types de boids
 * (proies, prédateurs, etc.). Chaque sous-classe devra juste implémenter step().
 */
public abstract class AbstractBoidSystem {
     ...
     * Les boids sont placés aléatoirement dans la zone de simulation au départ.
     ...
     * @param maxForce la force de steering maximale (pour des mouvements fluides)
     * @param fieldOfView l'angle de vision en radians (par exemple Math.PI * 0.75 pour 135°)
     * @param cohesionWeight le poids (importance) de la règle de cohésion
     * @param alignmentWeight le poids de la règle d'alignement
     * @param separationWeight le poids de la règle de séparation
     */
    public AbstractBoidSystem(int nbBoids, int width, int height,
        ...
        this.otherSystems = new ArrayList<>();
