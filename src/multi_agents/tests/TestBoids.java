...
public class TestBoids {
    ...
    public static void main(String[] args) {
        ...
        double maxForce = 0.15; // Force de steering maximale pour des mouvements fluides
        double fieldOfView = Math.PI * 0.8; // 144 degrés (vision assez large)
        double cohesionWeight = 1.0; // Poids de la cohésion
        double alignmentWeight = 1.0; // Poids de l'alignement
        double separationWeight = 1.5; // Poids de la séparation (un peu plus fort)

        // On crée un système de proies avec ces paramètres
        PreyBoidSystem preySystem = new PreyBoidSystem(
            nbBoids, width, height,
            rayonVision, distanceSep, Vmax, maxForce, fieldOfView,
